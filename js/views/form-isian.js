...
async function renderFormObservasi(root, s) {
  ...
  root.innerHTML = `
    <div class="screen">
      ...
      <div class="btn-row">
        ...
        <button class="btn btn-primary" id="btn-submit">${s.editMode ? 'Simpan Perubahan' : 'Submit Instrumen ' + instrumen}</button>
      </div>
    </div>
  `;
  ...
  document.getElementById('btn-submit').onclick = async function () {
    ...
    try {
      ...
      const fn = s.editMode
        ? (instrumen === 'B1' ? function (id, jw) { return apiUpdateB1(s.idIsian, id, jw); } : function (id, jw) { return apiUpdateB2(s.idIsian, id, jw); })
        : (instrumen === 'B1' ? apiSubmitB1 : apiSubmitB2);
      const data = await fn(identity, s.jawaban);
      s.hasil = data;
      saveDraft(s);
      App.lastKonfirmasi = { tipe: 'instrumen', instrumen: instrumen, mode: s.editMode ? 'edit' : 'baru', data: data };
      goTo('#/konfirmasi');
    } catch (e) {
      alert('Gagal submit: ' + e.message);
      btn.disabled = false;
      btn.textContent = s.editMode ? 'Simpan Perubahan' : 'Submit Instrumen ' + instrumen;
    }
  };
}
