...
Views.renderChecklistInstrumen = async function (root) {
  ...
  root.innerHTML = '<div class="screen"><div class="loading-wrap"><span class="spinner"></span> Memuat status instrumen...</div></div>';

  let pertB1, pertB2;
  try {
    const hasil = await Promise.all([cachedPertanyaan('B1'), cachedPertanyaan('B2')]);
    pertB1 = hasil[0];
    pertB2 = hasil[1];
  } catch (e) {
  ...
  ['B1', 'B2'].forEach(function (kode) {
    ...
    rows += '<div class="list-row" data-instrumen="' + kode + '">' +
      ...
      '<div class="instrumen-info">' + escapeHtml(INSTRUMEN_INFO[kode].targetRole) + '</div></div>' +
