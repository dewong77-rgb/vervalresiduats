...
Views.renderDaftarKegiatan = async function (root) {
  ...
  root.innerHTML = `
    <div class="screen">
      ...
      <div class="list-row-sub" style="margin-bottom:4px;">${escapeHtml(s.lokus.provinsi)}</div>
      <div class="list-row-sub" style="margin-bottom:16px;">Diisi oleh: ${escapeHtml(s.pengisi.nama)}${s.pengisi.jabatan ? ' (' + escapeHtml(s.pengisi.jabatan) + ')' : ''}</div>
      ...
      <div class="btn-row">
        ...
        <button class="btn btn-primary" id="btn-submit" ${s.kegiatan.length === 0 ? 'disabled' : ''}>${s.editMode ? 'Simpan Perubahan' : 'Submit Semua Kegiatan'}</button>
      </div>
    </div>
  `;
    ...
    btnSubmit.onclick = async function () {
      ...
      try {
        ...
        const kegiatanPayload = s.kegiatan.map(function (k) {
          const payload = Object.assign({}, k.jawaban);
          if (k.idIsian) payload.idIsian = k.idIsian;
          return payload;
        });
        const fn = s.editMode ? apiUpdateRTL : apiSubmitRTL;
        const data = await fn(identity, kegiatanPayload);
        s.hasil = data;
        saveDraft(s);
        if (data.file && data.file.base64) {
          downloadBase64Xlsx(data.file.fileName, data.file.base64);
        }
        App.lastKonfirmasi = { tipe: 'rtl', mode: s.editMode ? 'edit' : 'baru', data: data };
        goTo('#/konfirmasi');
      } catch (e) {
        alert('Gagal submit: ' + e.message);
        btnSubmit.disabled = false;
        btnSubmit.textContent = s.editMode ? 'Simpan Perubahan' : 'Submit Semua Kegiatan';
      }
    };
  }
};
