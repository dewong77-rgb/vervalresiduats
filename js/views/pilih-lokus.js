...
function identityKosong() {
  return {
    namaResponden: '', jabatanResponden: '', jumlahSiswa: '',
    namaPetugas: '', instansiPetugas: '', petugasLainnya: false
  };
}
...
Views.renderPilihLokus = async function (root) {
  root.innerHTML = `
    <div class="screen">
      ...
      <div class="card">
        ...
        <div id="area-dinamis"><p class="empty-hint">Memuat provinsi...</p></div>
      </div>
      <div class="btn-row">
  ...
  try {
    provinsiList = await apiGetProvinsi();
  } catch (e) {
  ...
  function renderAreaObservasi() {
    ...
    async function loadKabKota() {
      ...
      try {
        const raw = await apiGetKabKota(provSel.value);
    ...
    async function loadSekolah() {
      ...
      manualBox.style.display = 'none';
      try {
        const data = await apiGetSekolah(provSel.value, kabSel.value);
